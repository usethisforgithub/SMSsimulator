...
public class ScreenWindow extends Frame implements WindowListener, Runnable, KeyListener, MouseListener {
	...
	private Color[] colorArray = { Color.blue, Color.red, Color.green, Color.pink, Color.ORANGE, Color.cyan,
	...
	public ScreenWindow(int r, int c, int d, double a) {
		...
		for (Trajectory e : listTraj) {
			...
			for (Trajectory f : listTraj) {
				if (f.getVertex().geti() == e.getVertex().geti()
						&& f.getVertex().getj() == e.getVertex().getj() - trajSize) {
					e.addTop(f);
				}
				if (f.getVertex().geti() == e.getVertex().geti() + trajSize
						&& f.getVertex().getj() == e.getVertex().getj()) {
					e.addRight(f);
				}
				if (f.getVertex().geti() == e.getVertex().geti() - trajSize
						&& f.getVertex().getj() == e.getVertex().getj()) {
		...
		for (Arc e : allArcs) {
			if (!e.isAssigned()) {
				...
				while (unfinished) {
					...
					if (Utilities.radianEq(robot.getAngle(), 2 * Math.PI)
							|| Utilities.radianEq(robot.getAngle(), Math.PI / 2)
							|| Utilities.radianEq(robot.getAngle(), Math.PI)
							|| Utilities.radianEq(robot.getAngle(), 3 * (Math.PI / 2))
							|| Utilities.radianEq(robot.getAngle(), -Math.PI / 2)
							|| Utilities.radianEq(robot.getAngle(), -Math.PI)
							|| Utilities.radianEq(robot.getAngle(), -3 * (Math.PI / 2))
							|| Utilities.radianEq(robot.getAngle(), -2 * Math.PI)) {

						robot.setSensing(true);
						// System.out.println("1 " + e.get);

						// if drone is at left position
						if ((Utilities.radianEq(robot.getAngle(), Math.PI)
								|| Utilities.radianEq(robot.getAngle(), -Math.PI)) && !robot.getFlipped()) {
						...
						if ((Utilities.radianEq(robot.getAngle(), 2 * Math.PI)
								|| Utilities.radianEq(robot.getAngle(), -2 * Math.PI)
								|| Utilities.radianEq(robot.getAngle(), 0)) && !robot.getFlipped()) {
						...
						if ((Utilities.radianEq(robot.getAngle(), (Math.PI / 2))
								|| Utilities.radianEq(robot.getAngle(), -3 * (Math.PI / 2))) && !robot.getFlipped()) {
	...
	public void run() {
		while (isRunning) {
			...
			for (Robot e : listBot) {
				...
				// if at sensing angle
				if (Utilities.radianEq(e.getAngle(), 2 * Math.PI) || Utilities.radianEq(e.getAngle(), Math.PI / 2)
						|| Utilities.radianEq(e.getAngle(), Math.PI)
						|| Utilities.radianEq(e.getAngle(), 3 * (Math.PI / 2))
						|| Utilities.radianEq(e.getAngle(), -Math.PI / 2) || Utilities.radianEq(e.getAngle(), -Math.PI)
						|| Utilities.radianEq(e.getAngle(), -3 * (Math.PI / 2))
						|| Utilities.radianEq(e.getAngle(), -2 * Math.PI)) {
					...
					if ((Utilities.radianEq(e.getAngle(), 2 * Math.PI) || Utilities.radianEq(e.getAngle(), -2 * Math.PI)
							|| Utilities.radianEq(e.getAngle(), 0)) && !e.getFlipped()) {
						// if no drone to the right
					...
					if ((Utilities.radianEq(e.getAngle(), (Math.PI / 2))
							|| Utilities.radianEq(e.getAngle(), -3 * (Math.PI / 2))) && !e.getFlipped()) {
						// if no drone to the top
